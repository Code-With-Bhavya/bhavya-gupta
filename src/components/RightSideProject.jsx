...
const RightProject = ({ project }) => {
    ...
    return (
        <article className='relative w-full flex justify-center items-center flex-col-reverse lg:flex-row h-[100vh] overflow-y-hidden'>
            {/* Right Section (Now Left) */}
            <motion.section
                initial={{ y: 200, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.1, ease: 'easeInOut', duration: 0.5 }}
                className="relative w-full h-[20rem] lg:w-[50rem] lg:h-[50rem] lg:block flex flex-col justify-center overflow-y-hidden">
                {/* Circular Gradients */}
                <div className="absolute top-0 left-0 w-full h-full flex justify-between overflow-y-hidden md:overflow-y-visible p-1 lg:p-0">
                ...
                <figure className="lg:absolute  lg:top-1/2 lg:left-0 lg:-translate-y-1/2 overflow-y-hidden md:overflow-y-visible">
                    <div className="bg-[#fd6e001d] rounded-lg pt-5 pl-5 overflow-y-hidden md:overflow-y-visible">
            ...
            <motion.section
                ...
                transition={{ delay: 0.1, ease: 'easeInOut', duration: 0.5 }}
                className="relative w-full lg:w-[35.875rem] h-[19.375rem] flex justify-center flex-col lg:block gap-10 lg:gap-0 overflow-y-hidden md:overflow-y-visible">
                ...
                <div className='flex flex-col gap-5 lg:gap-0 lg:block'>
                    ...
                    {/* Tags*/}
                    <ul className="lg:absolute lg:bottom-2 ml-auto lg:ml-0 lg:right-10 flex gap-4 z-0">
                        {(project.tags).map((tag, index) => (
                            <motion.li
                                key={index}
                                whileHover={{ scale: 1.1 }}
                                className='text-white font-semibold rounded bg-[#FD6F00] p-1 lg:p-2 text-sm lg:text-base'>{tag}</motion.li>
                        ))}
                    </ul>
                </div>
