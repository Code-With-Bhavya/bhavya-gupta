...
const LeftProject = ({ project }) => {
    ...
    return (
        <article className='relative w-full flex justify-center items-center flex-col lg:flex-row h-full'>
            <motion.section
                initial={{ x: -100, opacity: 0 }}
                whileInView={{ x: 0, opacity: 1 }}
                transition={{ delay: 0.1, ease: 'easeInOut', duration: 0.5 }}
                className="relative w-full lg:w-[35.875rem] lg:h-[19.375rem] flex justify-center flex-col lg:block gap-10 lg:gap-0 "> {/*Left Section */}

                <header className='lg:absolute lg:top-0'> {/* Heading */}
                ...
                <div className='flex flex-col gap-5 lg:gap-0 lg:block'>
                    {/* Description Box  */}
                    <div className="lg:absolute lg:top-1/2 lg:-translate-y-1/2 lg:left-2  z-10 " >
                    ...
                    {/* Tags*/}
                    <ul className="lg:absolute lg:bottom-2 lg:left-10 flex gap-4 z-0 ">
                        {(project.tags).map((tag, index) => (
                            <motion.li
                                key={index}
                                whileHover={{ scale: 1.1 }}
                                className='text-white font-semibold rounded bg-[#FD6F00] p-1 lg:p-2'>{tag}
                            </motion.li>
                        ))}
                    </ul>
                </div>
            ...
            <motion.section
                ...
                transition={{ delay: 0.1, ease: 'easeInOut', duration: 0.5 }}
                className="relative w-full lg:w-[50rem] h-[20rem] lg:h-[50rem] lg:block flex flex-col justify-center">  {/* Right Section */}
                {/* Circular Gradients */}
                ...
                <figure className="lg:absolute lg:top-1/2 lg:right-0 lg:-translate-y-1/2">
